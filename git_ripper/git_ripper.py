    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise ValueError(
                f"file {str(self.download_directory)!r} is not directory!"
            )
    ...
    async def handle_download(
        ...
        if not downloaded.exists():
            try:
                ...
                logger.error("download failed: %s", download_url)
                if downloaded.exists():
                    downloaded.unlink()
                return
        else:
            logger.debug('file already exists: %s', downloaded)

        if filepath == 'config':
            logger.debug('parse config: %s', downloaded)
            ...
            r'COMMIT_EDITMSG|description|info/exclude|objects/.+', filepath
        ):
            """
            Примеры файлов
            ...
            packed-refs:
                ...
                ...
            """
            logger.debug("parse object hashes and refs: %s", downloaded)
            with downloaded.open() as f:
                contents = f.read()
